...
pub fn check_eligibility(
    ...
    if policy_satisfied(age, min_age, income, min_income, region, allowed_region) {
        1
    } else {
        0
    }
...
fn policy_satisfied(
    ...
    let mut ok = age >= min_age; // range proof: age gate
    ok &= region == allowed_region; // attribute match: region gate
    ok &= income >= min_income; // threshold: income floor
    ok
}
