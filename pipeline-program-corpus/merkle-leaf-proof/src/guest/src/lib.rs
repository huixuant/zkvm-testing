...
fn harness(input: Vec<u8>) -> Vec<u8> {
    // (1) DECODE ---------------------------------------------------------------
    // Byte layout: 1 × u32 little-endian = 4 bytes.
    //   [0..4)  leaves_base_2 : u32 LE  (tree has 2^leaves_base_2 leaves)
    // Short input -> empty output (total decode, never panics here).
    if input.len() < 4 {
        return Vec::new();
    }
    let leaves_base_2 = u32::from_le_bytes(input[0..4].try_into().unwrap());

    // (2) CALL -----------------------------------------------------------------
    let r = generate_merkle_proof(leaves_base_2);

    // (3) ENCODE ---------------------------------------------------------------
    // Return value is u32; encode as 4 bytes little-endian.
