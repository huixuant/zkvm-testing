...
/// FIXED, crate-generic harness signature — copied through verbatim (see role
/// HARD RULE / NOTES.md). Do not change the attribute or the signature.
///
/// DECODE byte layout: the code under test (`hashbench`) takes NO arguments and
/// derives all inputs internally from fixed seeds, so ZERO input bytes are
/// consumed and `input` is ignored. The harness is total for any input length
/// (including empty). Detection of the miscompilation is via assert-driven
/// panic / instrumentation, not via a data-dependent output.
///
/// ENCODE: the lifted body returns `[u8; 32]` (always all-zero, exactly the
/// original `hashbench` return value); those 32 bytes are emitted as output.
#[jolt::provable(max_output_size = 4096,
    heap_size = 33554432,
    stack_size = 10485760,
    max_trace_length = 50000000)]
fn harness(input: Vec<u8>) -> Vec<u8> {
    // (1) DECODE — no arguments to reconstruct; input is intentionally unused.
    let _ = &input;

    // (2) CALL — lifted body of the original `#[jolt::provable] fn hashbench()`.
    let result: [u8; 32] = hashbench_body();

    // (3) ENCODE — serialise the [u8; 32] result.
    let mut out = Vec::with_capacity(32);
    out.extend_from_slice(&result);
    out
}

/// Lifted verbatim from the original `hashbench()` provable body.
fn hashbench_body() -> [u8; 32] {
    bench_sha256();
    bench_keccak();
    bench_blake2b();
    bench_blake3();
